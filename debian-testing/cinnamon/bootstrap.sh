...
proot-distro login debian-testing-cinnamon --user debian-cinnamon' >> /data/data/com.termux/files/usr/bin/debian-testing-cinnamon

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/debian-testing-cinnamon-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
export XDG_RUNTIME_DIR=${TMPDIR}
kill -9 $(pgrep -f "termux.x11") 2>/dev/null
proot-distro login debian-testing-cinnamon --shared-tmp -- /bin/sh -c 'kill -9 $(pgrep -f "x11") 2>/dev/null'
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login debian-testing-cinnamon --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - debian-cinnamon -c "DISPLAY=:0 XDG_SESSION_TYPE=x11 dbus-launch --exit-with-session cinnamon-session"'
EOF
...
proot-distro login debian-testing-cinnamon -- /bin/sh -c 'wget https://raw.githubusercontent.com/arfshl/proot-distro-desktop/refs/heads/main/debian-testing/cinnamon/install.sh -O install.sh && chmod +x install.sh && ./install.sh && rm install.sh'
