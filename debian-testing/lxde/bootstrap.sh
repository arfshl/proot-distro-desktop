...
proot-distro login debian-testing-lxde --shared-tmp -- /bin/sh -c 'kill -9 $(pgrep -f "x11") 2>/dev/null'
...
proot-distro login debian-testing-lxde -- /bin/sh -c 'wget https://raw.githubusercontent.com/arfshl/proot-distro-desktop/refs/heads/main/debian-testing/lxde/install.sh -O install.sh && chmod +x install.sh && ./install.sh && rm install.sh'
