...
proot-distro login trisquel-lxde --user trisquel-lxde' >> /data/data/com.termux/files/usr/bin/trisquel-lxde

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/trisquel-lxde-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
proot-distro login trisquel-lxde --shared-tmp -- /bin/sh -c 'pkill -x -9 x11 2>/dev/null'
virgl_test_server_android &
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login trisquel-lxde --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - trisquel-lxde -c "DISPLAY=:0 GALLIUM_DRIVER=virpipe dbus-launch --exit-with-session startlxde"'
EOF

# Make all of them executable
chmod +x /data/data/com.termux/files/usr/bin/trisquel-lxde*

# Install trisquel under aliases
proot-distro install ghcr.io/arfshl/trisquel-docker:latest --name trisquel-lxde
...
echo 'Default user: trisquel-lxde'
