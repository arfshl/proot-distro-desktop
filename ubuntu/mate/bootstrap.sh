...
proot-distro login ubuntu-mate --shared-tmp -- /bin/sh -c 'pkill -x -9 x11 2>/dev/null'
...
proot-distro login ubuntu-mate -- /bin/sh -c 'wget https://raw.githubusercontent.com/arfshl/proot-distro-desktop/refs/heads/main/ubuntu/mate/install.sh -O install.sh && chmod +x install.sh && ./install.sh'

echo 'To start command line session: ubuntu-mate'
echo 'To start X11 session: ubuntu-mate-x11'
