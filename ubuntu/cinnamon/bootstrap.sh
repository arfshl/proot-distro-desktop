...
proot-distro login ubuntu-cinnamon --shared-tmp -- /bin/sh -c 'pkill -x -9 x11 2>/dev/null'
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login ubuntu-cinnamon --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - ubuntu-cinnamon -c "DISPLAY=:0 XDG_SESSION_TYPE=x11 dbus-launch --exit-with-session cinnamon-session"'
EOF
...
proot-distro install ubuntu:latest --name ubuntu-cinnamon

# Setup ubuntu-cinnamon
proot-distro login ubuntu-cinnamon -- /bin/sh -c 'apt update && apt install wget -y'

proot-distro login ubuntu-cinnamon -- /bin/sh -c 'wget https://raw.githubusercontent.com/arfshl/proot-distro-desktop/refs/heads/main/ubuntu/cinnamon/install.sh -O install.sh && chmod +x install.sh && ./install.sh'
