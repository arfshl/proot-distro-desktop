...
proot-distro login void-lxde --user void-lxde' >> /data/data/com.termux/files/usr/bin/void-lxde

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/void-lxde-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
sleep 3
proot-distro login void-lxde --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - void-lxde -c "DISPLAY=:0 GALLIUM_DRIVER=virpipe dbus-launch --exit-with-session startlxde"'
EOF
...
proot-distro install void --override-alias void-lxde

# Setup void-lxde
proot-distro login void-lxde -- /bin/sh -c 'xbps-install -Su && xbps-install -S wget -y'

proot-distro login void-lxde -- /bin/sh -c 'wget https://raw.githubusercontent.com/arfshl/proot-distro-desktop/refs/heads/main/void/lxde/install.sh -O install.sh && chmod +x install.sh && ./install.sh && rm install.sh'

echo 'To start command line session: void-lxde'
echo 'To start X11 session: void-lxde-x11'
