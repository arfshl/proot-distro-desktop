...
proot-distro login void-lxqt --user void-lxqt' >> /data/data/com.termux/files/usr/bin/void-lxqt

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/void-lxqt-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
proot-distro login void-lxqt --shared-tmp -- /bin/sh -c 'pkill -x -9 x11 2>/dev/null'
virgl_test_server_android &
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login void-lxqt --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - void-lxqt -c "DISPLAY=:0 GALLIUM_DRIVER=virpipe dbus-launch --exit-with-session startlxqt"'
EOF
...
proot-distro install ghcr.io/void-linux/void-musl:latest --name void-lxqt
