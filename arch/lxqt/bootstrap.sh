...
proot-distro login arch-lxqt --user arch-lxqt' >> /data/data/com.termux/files/usr/bin/arch-lxqt

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/arch-lxqt-x11
#!/usr/bin/env bash
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
proot-distro login arch-lxqt --shared-tmp -- /bin/sh -c 'pkill -x -9 x11 2>/dev/null'
virgl_test_server_android &
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login arch-lxqt --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - arch-lxqt -c "DISPLAY=:0 GALLIUM_DRIVER=virpipe dbus-launch --exit-with-session startlxqt"'
EOF
...
if [ "$ARCH" = "x86_64" ] || [ "$ARCH" = "i686" ]; then 
    ...
    proot-distro install arfshl/archlinuxarm:latest --name arch-lxqt
else
...
proot-distro login arch-lxqt -- /bin/sh -c 'wget https://raw.githubusercontent.com/arfshl/proot-distro-desktop/refs/heads/main/arch/lxqt/install.sh -O install.sh && chmod +x install.sh && ./install.sh'
