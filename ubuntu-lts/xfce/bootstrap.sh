...
proot-distro install ubuntu:26.04 --name ubuntu-lts-xfce

# Setup ubuntu-xfce
proot-distro login ubuntu-lts-xfce -- /bin/sh -c 'apt update && apt install wget -y'

proot-distro login ubuntu-lts-xfce -- /bin/sh -c 'wget https://raw.githubusercontent.com/arfshl/proot-distro-desktop/refs/heads/main/ubuntu/xfce/install.sh -O install.sh && chmod +x install.sh && ./install.sh'
...
proot-distro login ubuntu-lts-xfce --shared-tmp -- /bin/sh -c 'pkill -x -9 x11 2>/dev/null'
