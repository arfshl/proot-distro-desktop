...
proot-distro login ubuntu-lts-xfce --user ubuntu-xfce' >> /data/data/com.termux/files/usr/bin/ubuntu-lts-xfce

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/ubuntu-lts-xfce-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
proot-distro login ubuntu-xfce --shared-tmp -- /bin/sh -c 'kill -9 $(pgrep -f "x11") 2>/dev/null'
virgl_test_server_android &
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login ubuntu-lts-xfce --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - ubuntu-xfce -c "DISPLAY=:0 GALLIUM_DRIVER=virpipe dbus-launch --exit-with-session startxfce4"'
EOF
...
PD_OVERRIDE_TARBALL_URL="https://github.com/arfshl/pd-custom-rootfs/releases/download/ubuntu-lts-latest/ubuntu-lts-latest-arm64.tar.xz" PD_OVERRIDE_TARBALL_SHA256="6db3c0d14b52bf2d78380631593774b779396893f8e25fc8a5ead4d3d68c2562" proot-distro install ubuntu --override-alias ubuntu-xfce
