...
proot-distro login manjaro-cinnamon --user manjaro-cinnamon' >> /data/data/com.termux/files/usr/bin/manjaro-cinnamon

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/manjaro-cinnamon-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
proot-distro login manjaro-cinnamon --shared-tmp -- /bin/sh -c 'kill -9 $(pgrep -f "x11") 2>/dev/null'
virgl_test_server_android &
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login manjaro-cinnamon --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - manjaro-cinnamon -c "DISPLAY=:0 XDG_SESSION_TYPE=x11 dbus-launch --exit-with-session cinnamon-session"'
EOF
