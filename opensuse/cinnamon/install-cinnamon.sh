...
sudo useradd -m opensuse-cinnamon && echo 'opensuse-cinnamon:123' | chpasswd && echo 'opensuse-cinnamon ALL=(ALL:ALL) ALL' >> /etc/sudoers.d/user


# Setup VNC server
# Create VNC configuration directory
mkdir -p /home/opensuse-cinnamon/.vnc

# Create VNC password file (default 1234567890)
printf "1234567890" | vncpasswd -f > /home/opensuse-cinnamon/.vnc/passwd
chmod 600 /home/opensuse-cinnamon/.vnc/passwd
...
kill -9 $(pgrep -f "cinnamon") 2>/dev/null
rm -rf /tmp/.X0-lock
...
chmod +x restartvnc
cd
# chmod +x /home/opensuse-cinnamon/.vnc/xstartup
