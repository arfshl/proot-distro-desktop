...
proot-distro login artix-xfce --user artix-xfce' >> /data/data/com.termux/files/usr/bin/artix-xfce

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/artix-xfce-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
proot-distro login artix-xfce --shared-tmp -- /bin/sh -c 'pkill -x -9 x11 2>/dev/null'
virgl_test_server_android &
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login artix-xfce --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - artix-xfce -c "DISPLAY=:0 GALLIUM_DRIVER=virpipe dbus-launch --exit-with-session startxfce4"'
EOF
...
proot-distro login artix-xfce -- /bin/sh -c 'wget https://raw.githubusercontent.com/arfshl/proot-distro-desktop/refs/heads/main/arch/xfce/install.sh -O install.sh && chmod +x install.sh && ./install.sh'

echo 'To start command line session: artix-xfce'
echo 'To start X11 session: artix-xfce-x11'
