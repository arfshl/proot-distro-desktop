...
proot-distro login artix-mate --user artix-mate' >> /data/data/com.termux/files/usr/bin/artix-mate

# for X11 session
cat <<'EOF' > /data/data/com.termux/files/usr/bin/artix-mate-x11
#!/bin/sh
LD_PRELOAD=/system/lib64/libskcodec.so
pulseaudio --start --load="module-native-protocol-tcp auth-ip-acl=127.0.0.1 auth-anonymous=1" --exit-idle-time=-1
...
proot-distro login artix-mate --shared-tmp -- /bin/sh -c 'kill -9 $(pgrep -f "x11") 2>/dev/null'
virgl_test_server_android &
termux-x11 :0 >/dev/null &
sleep 3
proot-distro login artix-mate --shared-tmp -- /bin/sh -c 'export PULSE_SERVER=127.0.0.1 && export XDG_RUNTIME_DIR=${TMPDIR} && su - artix-mate -c "DISPLAY=:0 GALLIUM_DRIVER=virpipe dbus-launch --exit-with-session mate-session"'
EOF
...
# Setup artix-mate
proot-distro login artix-mate -- /bin/sh -c 'apt update && apt install wget -y'
